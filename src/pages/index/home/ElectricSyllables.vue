...
export default {
  ...
  methods: {
    ...
    goReport(e) {
      this.$router.push({ path: "/report", query: { postId: e.target.dataset.postId } });
    },
    loadData() {
          //pageSize, currentPage, postPlate, userId, postId, isAttention
      getQueryAllPost(this.pageSize, this.currentPage,this.postType, this.userId,'',2)
        .then(res => {
          if (res.code === 200) {
  ...
  created() {
    getQueryAllPost(4, 1, this.postType, 1, "", 2)
      .then(res => {
        if (res.code == 200) {
